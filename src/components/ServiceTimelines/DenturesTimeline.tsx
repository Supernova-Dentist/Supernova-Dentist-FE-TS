...
export const DenturesTimeline = ({ data }: TimelineProps) => {
  ...
  return (
    <div id='dentures-timeline' className='w-full font-sans xl:px-10' ref={containerRef}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <div className='max-w-7xl mx-auto py-20 px-4 xl:px-8 lg:px-10'>
          ...
          <p className='text-xl xl:text-2xl lg:text-2xl text-center text-gray-800 max-w-[35rem] mx-auto'>
            Replacing missing teeth with dentures is a straightforward process designed to restore your smile,
            confidence, and ability to eat comfortably. Here&apos;s what to expect:
          </p>
        </div>
      ...
      <div className='flex justify-center pb-6'>
        <Link href={`/enquiry?ref=${encodeURIComponent('general-dentistry/dentures')}`}>
          <button className='bg-gold text-white px-8 py-4 rounded-lg text-lg font-semibold'>
            Book Your Dentures Consultation
          </button>
        </Link>
      </div>
    </div>
