...
export const ImplantInvisalignBanner = ({
  onDismiss,
  className,
  children,
  hideOnScroll = false,
  ...
  useMotionValueEvent(scrollY, 'change', (latest) => {
    if (hideOnScroll && latest > 40) {
      setOpen(false);
    } else if (!localStorage.getItem('implantinvisalignbannerDismissed')) {
      setOpen(true);
    }
  });
  ...
  const handleClose = () => {
    setOpen(false);
    localStorage.setItem('implantinvisalignbannerDismissed', 'true');
