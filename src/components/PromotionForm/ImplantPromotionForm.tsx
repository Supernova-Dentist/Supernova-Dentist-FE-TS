  ...
  async function onSubmit(data: PromotionFormData) {
    try {
      ...
      const dataWithTracking = {
        ...
        tracking: buildSubmissionTracking({ form: 'implant-consultation', service: cleanedSource }),
      };
      ...
      const eventName = alreadyExists ? 'ExistingImplantLead' : 'NewImplantLead';
          ...
          <div className='container mx-auto max-w-[1200px] grid lg:grid-cols-2 gap-10 items-center px-2'>
            ...
            <Card className='w-full max-w-lg mx-auto bg-white shadow-2xl border p-4 md:p-8 rounded-2xl'>
              <CardHeader className='text-center p-0 mb-2'>
                <CardTitle id='implant-enquiry-title' className='text-3xl font-bold'>Start Your Implant Consultation Enquiry</CardTitle>

                <p className='text-sm text-gray-600 mt-2'>Tell the practice team what you would like to discuss</p>
              </CardHeader>
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-6' noValidate aria-labelledby='implant-enquiry-title'>
                  {/* INTENT SELECTOR */}
                  ...
                  <div className='flex items-center mt-2'>
                    <Checkbox
                      id='implant-opt-out-emails'
                      checked={form.watch('optOutEmails')}
                      onCheckedChange={(checked) => form.setValue('optOutEmails', checked === true)}
                    />
                    <Label htmlFor='implant-opt-out-emails' className='ml-3 text-sm text-gray-500'>
                  ...
                  <Button type='submit' disabled={form.formState.isSubmitting} aria-busy={form.formState.isSubmitting} className='w-full bg-gold hover:bg-lightGold text-lg py-3'>
                    {form.formState.isSubmitting ? <BarLoader /> : 'Send Implant Enquiry'}
                  </Button>
                  ...
                  <div className='text-sm text-gray-600 space-y-1'>
                    <p>Share your tooth-replacement needs or questions</p>
