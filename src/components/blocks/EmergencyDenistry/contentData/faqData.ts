export const faqItems: FAQItem[] = [
  {
    question: 'What qualifies as a dental emergency?',
    answer:
      'A dental emergency can include severe toothache, knocked-out or broken teeth, lost fillings or crowns, uncontrollable bleeding, swelling or infections, and any injury causing significant oral pain or risk of further damage.',
  },
  {
    question: 'How quickly can I get an emergency appointment?',
    answer:
      'We aim to see genuine emergencies on the same day whenever possible. Please call us during opening hours to secure your slot immediately.',
  },
  {
    question: 'Do you offer out-of-hours emergency care?',
    answer:
      'We do not offer out-of-hours appointments, but you can contact us via WhatsApp at 07863 338815 for urgent advice. We’ll assess your situation as soon as we can and arrange treatment during our next available hours.',
  },
  {
    question: 'What should I do if I knock out a tooth?',
    answer:
      'Keep the tooth moist (in milk or saliva), avoid touching the root, and contact us immediately. Fast action can often allow us to re-implant the tooth successfully.',
  },
  {
    question: 'Is there an upfront cost for emergency treatment?',
    answer:
      'Yes. For non-registered patients, we charge £100 for emergency appointments this contributes toward any treatment needed on the day. For registered patients, the emergency appointments fee is £65. Any additional treatment costs will be clearly explained before proceeding.',
  },

  {
    question: 'Can I book an emergency appointment through the patient portal?',
    answer:
      'Yes, you can book emergency appointments via our patient portal. We advise filling out the triage form before your appointment to help us provide more efficient emergency dental care.',
  },
];
