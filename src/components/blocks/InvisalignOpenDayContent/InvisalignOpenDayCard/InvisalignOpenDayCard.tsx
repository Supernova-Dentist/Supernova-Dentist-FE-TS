...
export default function InvisalignOpenDayCard({
  ...
  return (
    ...
    >
      <img src='/favicon.ico' alt='Supernova Dental Logo' className='absolute top-4 right-4 h-16 w-auto' /> {/* Slightly larger logo */}
      <div className='flex flex-col flex-grow'>
        ...
        <span className={cn('text-6xl mb-2 font-semibold', isMiddle ? 'text-gray-50' : null)}> {/* Increased font size */}
          £
          <CountUp
            className=''
            start={price.number}
        ...
        <div className={cn('bg-gray-200 w-full h-[1px] my-10', isMiddle ? 'bg-gray-700' : 'bg-gray-200')}></div> {/* Increased spacing */}

        <ul className='flex flex-col gap-4'> {/* Increased gap */}
          {features.map((feature) => (
            <li className={cn('flex gap-2 items-center', isMiddle ? 'text-gray-50' : null)} key={feature}> {/* Increased gap */}
              <IoCheckmarkCircle color={isMiddle ? 'var(--lightGold)' : ''} size={24} /> {/* Larger icon */}
              {feature}
            </li>
          ))}
        </ul>
      ...
      <div className='mt-auto'>
        ...
        <Button
          ...
          className={cn('w-full text-gray-50 rounded-sm mt-8 px-6 py-3', isMiddle ? 'bg-lightGold' : 'bg-gray-900')} 
          text='Claim offer'
        />
      </div>
    </motion.div>
