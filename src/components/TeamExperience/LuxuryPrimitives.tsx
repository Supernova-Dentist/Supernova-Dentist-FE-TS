...
export function LuxuryLink({
  ...
  return (
    <Link
      ...
      className={cn(
        'group inline-flex min-h-11 items-center justify-center gap-2 rounded-full border px-6 py-3 text-sm font-semibold tracking-[0.03em] transition-[background-color,border-color,color,box-shadow,transform] duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 active:scale-[0.98]',
        variant === 'primary' &&
        ...
        variant === 'light' &&
          'border-obsidian/20 bg-transparent text-obsidian hover:border-champagne hover:bg-champagne/10 focus-visible:ring-focus-light',
        variant !== 'light' && 'focus-visible:ring-focus-dark',
        className
      )}
    >
      {children}
...
export function BackToTeamLink({ className }: { className?: string }) {
  return (
    <Link
      href='/team'
      className={cn(
        'group inline-flex min-h-11 items-center gap-2 rounded-full text-sm font-semibold text-obsidian underline decoration-champagne/60 underline-offset-4 transition-colors duration-200 hover:decoration-champagne focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus-light focus-visible:ring-offset-4',
        className
      )}
    >
